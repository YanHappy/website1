...
if ( ! class_exists( 'WC_360_Image_Display' ) ) {
	...
	class WC_360_Image_Display {
		...
		public function load_scripts() {
			...
			wp_register_script( 'wc360-threesixty-js', plugins_url( 'lib/threesixty/js/threesixty' . $suffix . '.js', dirname( __FILE__ ) ), array( 'jquery' ), true );
			wp_register_script( 'wc360-threesixty-fullscreen-js', plugins_url( 'lib/threesixty/js/plugins/threesixty.fullscreen.js', dirname( __FILE__ ) ), array( 'jquery', 'wc360-threesixty-js' ), true );
			wp_register_style( 'wc360-threesixty-css', plugins_url( 'lib/threesixty/css/threesixty.css', dirname( __FILE__ ) ) );

			wp_register_script( 'wc360-js', plugins_url( 'assets/js/wc360.js', dirname( __FILE__ ) ), array( 'jquery' ), false, true );
			wp_register_style( 'wc360-css', plugins_url( 'assets/css/wc360.css', dirname( __FILE__ ) ) );
		...
		public function inline_styles() {
			...
			if ( $this->display_bool() == true ) {
				...
				$image_size = $this->image_size( $this->image_size_name() );

				// 360° Image Width
				$width = isset( $image_size['width'] ) ? $image_size['width'] : '500';
		...
		public function js_data_all( $shortcode_width = '', $shortcode_height = '', $post_id = '' ) {
			...
			$images_array = json_encode( $this->js_data_images( $post_id ) );
